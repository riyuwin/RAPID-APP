...
export const ResetForms = async () => {


    // Function to get the input value and reset the input
    const getValueInputIfNotEmpty = (inputId) => {
    ...
    // Function to get the checkbox value if checked and reset it
    const getCheckboxValueAndReset = (checkboxId) => {
        const checkboxElement = document.getElementById(checkboxId);
        if (checkboxElement && checkboxElement.checked) {
            checkboxElement.checked = false; // Reset the checkbox
            return true;
        }
        return false;
    };
    ...
    // Function to get the checkbox value if checked and reset it
    const getCheckboxValueIfChecked = (checkboxId) => {
        const checkboxElement = document.getElementById(checkboxId);
        if (checkboxElement && checkboxElement.checked) {
            checkboxElement.checked = false; // Reset the checkbox
            return true;
    ...
    const triageTagging = {};
    if (getCheckboxValueIfChecked('triageTaggingR')) triageTagging['triageTaggingR'] = getCheckboxValueIfChecked('triageTaggingR');
    if (getCheckboxValueIfChecked('triageTaggingY')) triageTagging['triageTaggingY'] = getCheckboxValueIfChecked('triageTaggingY');
    if (getCheckboxValueIfChecked('triageTaggingG')) triageTagging['triageTaggingG'] = getCheckboxValueIfChecked('triageTaggingG');
    if (getCheckboxValueIfChecked('triageTaggingB')) triageTagging['triageTaggingB'] = getCheckboxValueIfChecked('triageTaggingB');
    ...
    // Part 2 object
    const part2 = {};
    if (getValueInputIfNotEmpty('vehicularAccidentDetails')) {
        part2['vehicularAccidentDetails'] = getValueInputIfNotEmpty('vehicularAccidentDetails');
    }
    ...
    if (getValueInputIfNotEmpty('classificationGovernment')) {
        classification['classificationGovernment'] = getValueInputIfNotEmpty('classificationGovernment');
    }
    ...
    // Populate `part3` and reset input
    const part3 = {};
    if (getValueInputIfNotEmpty('part3')) part3['part3'] = getValueInputIfNotEmpty('part3');

    // Populate `involvement` and reset inputs and checkboxes
    const involvement = {};
    if (getCheckboxValueAndReset('involvement_driver')) involvement['involvement_driver'] = true;
    if (getCheckboxValueAndReset('involvement_passenger')) involvement['involvement_passenger'] = true;
    if (getCheckboxValueAndReset('involvement_pedestrian')) involvement['involvement_pedestrian'] = true;
    if (getValueInputIfNotEmpty('licenseNumber')) involvement['licenseNumber'] = getValueInputIfNotEmpty('licenseNumber');
    if (getCheckboxValueAndReset('NolicenseNumber')) involvement['NolicenseNumber'] = true;

    // Populate `driverError` and reset checkboxes
    const driverError = {};
    if (getCheckboxValueAndReset('driverErrorFatigued')) driverError['driverErrorFatigued'] = true;
    if (getCheckboxValueAndReset('driverErrorNoSignal')) driverError['driverErrorNoSignal'] = true;
    if (getCheckboxValueAndReset('driverErrorBadOvertaking')) driverError['driverErrorBadOvertaking'] = true;
    if (getCheckboxValueAndReset('driverErrorInattentive')) driverError['driverErrorInattentive'] = true;
    if (getCheckboxValueAndReset('driverErrorBadTurning')) driverError['driverErrorBadTurning'] = true;
    if (getCheckboxValueAndReset('driverErrorTooFast')) driverError['driverErrorTooFast'] = true;
    if (getCheckboxValueAndReset('driverErrorUsingCellphone')) driverError['driverErrorUsingCellphone'] = true;
    if (getCheckboxValueAndReset('driverErrorTooClose')) driverError['driverErrorTooClose'] = true;

    // Populate `injury` and reset checkboxes
    const injury = {};
    if (getCheckboxValueAndReset('injuryFatal')) injury['injuryFatal'] = true;
    if (getCheckboxValueAndReset('injurySerious')) injury['injurySerious'] = true;
    if (getCheckboxValueAndReset('injuryMinor')) injury['injuryMinor'] = true;
    if (getCheckboxValueAndReset('injuryNotInjured')) injury['injuryNotInjured'] = true;

    // Populate `alcoholDrugs` and reset checkboxes
    const alcoholDrugs = {};
    if (getCheckboxValueAndReset('alcoholSuspected')) alcoholDrugs['alcoholSuspected'] = true;
    if (getCheckboxValueAndReset('drugsSuspected')) alcoholDrugs['drugsSuspected'] = true;

    // Populate `seatbeltHelmet` and reset checkboxes
    const seatbeltHelmet = {};
    if (getCheckboxValueAndReset('seatbeltHelmetWorn')) seatbeltHelmet['seatbeltHelmetWorn'] = true;
    if (getCheckboxValueAndReset('seatbeltHelmetNotWorn')) seatbeltHelmet['seatbeltHelmetNotWorn'] = true;
    if (getCheckboxValueAndReset('seatbeltHelmetNotWornCorrectly')) seatbeltHelmet['seatbeltHelmetNotWornCorrectly'] = true;
    if (getCheckboxValueAndReset('seatbeltHelmetNoSeatbelt')) seatbeltHelmet['seatbeltHelmetNoSeatbelt'] = true;
